...
from bme2803 import process_bme_reading
    ...
    if found_bme_data:
        ...
        while (y<len(temp_queue)):
            ...
            if data.startswith('BMP'):
                ...
                if bme_messages==5:
                    bme2803=process_bme_reading(bme_data, devID)
                    if (bme2803.temp_rt and bme2803.hum_rt and bme2803.press_rt):
                      if bme2803.error != "":
                          print(bme2803.error)
                      else:
                          processing_queue.insert(len(processing_queue), (devID, "TMPA"+str(round(bme2803.temp,2))))
                          processing_queue.insert(len(processing_queue), (devID, "HUM"+str(round(bme2803.hum,2))))
                          processing_queue.insert(len(processing_queue), (devID, "PA"+str(round(bme2803.press/100,1))))
                bme_messages=0;
                bme_data=""
            else:
                y=y+1
